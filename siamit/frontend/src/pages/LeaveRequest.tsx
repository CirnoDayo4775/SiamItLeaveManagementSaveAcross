  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-indigo-50 to-white flex flex-col">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center py-6 md:py-16">
          <img
            ...
            alt={t('common.logo')}
            className="w-14 h-14 md:w-24 md:h-24 rounded-full bg-white/80 shadow-2xl border-4 border-white mb-4"
          />
          <h1 className="text-xl md:text-5xl font-extrabold text-indigo-900 drop-shadow mb-2 flex items-center gap-2 md:gap-3">
            <Send className="w-5 h-5 md:w-8 md:h-8 text-blue-600" aria-hidden="true" />
            {t('leave.leaveRequest')}
          </h1>
          <p className="text-xs md:text-xl text-blue-900/70 mb-2 font-medium text-center max-w-2xl px-4">
      ...
      <div className="w-full max-w-3xl mx-auto px-4 mt-0 animate-fade-in flex-1">
        <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-4 md:p-8">
          <LeaveForm mode="create" />
        </div>
      </div>

      {/* Footer */}
      <footer className="w-full mt-6 md:mt-16 py-6 md:py-8 bg-gradient-to-r from-blue-100 via-indigo-50 to-white text-center text-gray-400 text-base font-medium shadow-inner flex flex-col items-center gap-2">
        <img
          src="/lovable-uploads/siamit.png"
          alt={t('common.logo')}
          className="w-8 h-8 md:w-10 md:h-10 rounded-full mx-auto mb-1"
        />
